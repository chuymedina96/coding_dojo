...
create_apps = f"cd {projectTitle} && mkdir apps && cd apps && python ../manage.py startapp {set_up_app} && cd {set_up_app} && type nul > urls.py"
...
server = (f"cd {projectTitle} && python manage.py runserver")
os.system(server)
